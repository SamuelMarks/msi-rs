...
- [2. Command-Line Interface (CLI) Guide](#2-command-line-interface-cli-guide)
  - [Primary Tool: `msi` / `msi-cli`](#primary-tool-msi--msi-cli)
  ...
  - [Graphical (GUI) and Terminal (TUI) Installers](#graphical-gui-and-terminal-tui-installers)
    ...
    - [`msi-gui-tui`: Headless / SSH Terminal Wizard](#msi-gui-tui-headless--ssh-terminal-wizard)
...
git clone https://github.com/user/msi-rs.git
cd msi-rs
...
| :--- | :--- | :--- |
| `msi-cli` / `msi` | Main installer runtime, manager, and `msiexec` emulator | Microsoft `msiexec.exe` |
...
### Primary Tool: `msi` / `msi-cli`

The `msi-cli` executable (also aliased as `msi`) provides a modern subcommand interface alongside direct `msiexec.exe` flag parity.
...
#### `msi-gui-tui`: Headless / SSH Terminal Wizard
...
fn inspect_package(path: &Path) -> Result<()> {
    ...
    for rec in file_records {
        if let (Some(FieldValue::String(file_id)), Some(FieldValue::Integer(size))) =
            (rec.get(0), rec.get(3))
        {
...
print(json.dumps(info["summary_info"], indent=2))
  ...
  build-installer:
    ...
    steps:
      ...
      - name: Install msi-rs CLI tools
        run: |
          cargo install --git https://github.com/user/msi-rs.git msi-cli
